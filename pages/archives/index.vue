<template>
  <div class="container mx-auto px-4 py-8 max-w-5xl">
    ...
    <div v-if="archives.length > 0" class="space-y-8">
      <div v-for="item in archives" :key="`${item.year}-${item.month}`" class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-100 dark:border-gray-700">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">
          <router-link :to="`/archives/${item.year}/${item.month}`" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
        ...
        <ul class="mt-4 space-y-2">
          <li v-for="article in item.articles" :key="article.id">
            <router-link :to="`/${article.id}`" class="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:underline">
              {{ article.title }}
            </router-link>
