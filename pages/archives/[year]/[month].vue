<template>
  <div class="container mx-auto px-4 py-8 max-w-5xl">
    <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-8">
      {{ route.params.year }}年{{ route.params.month }}月 归档
    </h1>
    ...
    <div v-if="articles.length > 0" class="space-y-6">
      <div v-for="article in articles" :key="article.id"
        ...
        <div class="flex flex-wrap gap-2 mt-3 text-sm">
          <span class="text-gray-500 dark:text-gray-500">分类：{{ article.category }}</span>
          <span class="text-gray-500 dark:text-gray-500">标签：{{ article.tags.join(', ') }}</span>
        </div>
      </div>
    </div>
